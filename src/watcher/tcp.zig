...
fn TCPStream(comptime xev: type) type {
    return struct {
        ...
        /// Receive data and ancillary (cmsg) bytes via recvmsg(2) using
        /// caller-supplied payload and cmsg buffers. The library builds the
        /// msghdr internally; storage for msghdr+iov lives in the Completion.
        ///
        /// `flags` is passed through to the recvmsg flags argument (MSG_PEEK,
        /// MSG_TRUNC, etc.). 0 is the common case.
        ///
        /// On completion, the callback receives a RecvmsgCmsgResult with the
        /// payload byte count, the populated cmsg byte count, and the
        /// msghdr.flags (notably MSG_CTRUNC if cmsg_buf was too small).
        ///
        ...
        pub fn recvmsgCmsg(
            ...
            switch (xev.backend) {
                .io_uring => {
                    c.* = .{
                        .op = .{
                            .recvmsg_cmsg = .{
                                .fd = self.fd,
                                .flags = flags,
                                // msghdr + iov are filled in below so we can
                                // point at the inline storage by &c.op.
                                .msghdr = undefined,
                                .iov = undefined,
                            },
                        },
                        ...
                        .callback = (struct {
                            fn callback(
                                ...
                                const op = &c_inner.op.recvmsg_cmsg;
                                return @call(.always_inline, cb, .{
                                    common.userdataValue(Userdata, ud),
                                    l_inner,
                                    c_inner,
                    ...
                    // Populate msghdr/iov to point at the inline storage.
                    const op = &c.op.recvmsg_cmsg;
                    op.iov[0] = .{ .base = buf.ptr, .len = buf.len };
                    op.msghdr = std.mem.zeroes(posix.msghdr);
                ...
                .kqueue => @compileError("recvmsgCmsg is Linux/io_uring-only"),
            }
            loop.add(c);
        }
...
fn TCPTests(comptime xev: type, comptime Impl: type) type {
    return struct {
        ...
        test "TCP: recvmsgCmsg basic" {
            if (!@hasDecl(Impl, "recvmsgCmsg")) return;
            if (xev.backend != .io_uring) return;
            ...
            var got = Result{};
            var recv_buf: [128]u8 = undefined;
            var cmsg_buf: [64]u8 = undefined;
            var c_recv: xev.Completion = undefined;
            server_conn.?.recvmsgCmsg(
                &loop,
                &c_recv,
                &recv_buf,
                &cmsg_buf,
            ...
            try testing.expectEqual(send_buf.len, got.bytes);
            try testing.expectEqualSlices(u8, send_buf, recv_buf[0..got.bytes]);
            // No cmsg was sent, so no ancillary data should land in cmsg_buf.
